    ...
    return (
        <div
            className={cn('relative z-10 flex h-16 w-16 items-center justify-center rounded-full border-4 shadow-lg', {
                'border-green-200 bg-green-500': booking.status === 'confirmed' || booking.status === 'completed',
                'border-yellow-200 bg-yellow-500': booking.status === 'pending',
                'border-red-200 bg-red-500': booking.status === 'cancelled' || booking.status === 'no_show',
            })}
        >
